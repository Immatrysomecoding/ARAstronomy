...
public class OpenCVCameraManager : MonoBehaviour
{
    ...
    public bool enableDenoising = false;           // Can be slow
    ...
    IEnumerator InitializeSystem()
    {
        ...
        if (showDebugInfo)
        {
            Debug.Log("SIFT-based OpenCV Camera Manager initialized!");
        }
    }
    ...
    void InitializeCamera()
    {
        ...
        if (devices.Length > 0)
        {
            ...
            webCamTexture = new WebCamTexture(deviceName, requestedWidth, requestedHeight, 30);
            webCamTexture.Play();
    ...
    void InitializeOpenCV()
    {
        ...
        // Initialize FLANN matcher for SIFT (uses KDTree)
        using (var indexParams = new FlannIndexParams())
        using (var searchParams = new FlannSearchParams())
        {
            indexParams.setAlgorithm(0); // FLANN_INDEX_KDTREE
            indexParams.setInt("trees", 5);
            searchParams.setInt("checks", 50);
            
            flannMatcher = new FlannBasedMatcher(indexParams, searchParams);
        }
